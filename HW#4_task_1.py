#compute probablity VAR
from scipy.stats import norm
import numpy as np
import pandas as pd
import math

def VaR(Returns, Formula = 'Parametric Normal', Confidence_Interval = 0.95 ,
        Period_Interval = None , EWMA_Discount_Factor = 0.94, 
        Series = False ,removeNa = True):
    
    # Removes the NAs from the series
    if removeNa==True: Returns = Returns[pd.notnull(Returns)]
    
    # Data need to be returns already, 
    #then here the interval for the sampling is set for No input
...
def VaR_Compare(Returns, Confidence_Interval = 0.95 , 
                Period_Interval = 100 ,EWMA_Discount_Factor = 0.94):
...
df_us.tail()
us_open = df_us.iloc[:,2]
us_open = np.array(us_open)
nr = len(us_open)
op_ret = np.zeros(nr-1)
for i in range(nr):
    if i != (nr-1):
        op_ret[i] = ((us_open[i+1]/us_open[i])-1)*100
    else:
        op_ret[i-1] =0

#calling function
df_ret = pd.DataFrame(op_ret)
df_ret.tail()
var1 = VaR(df_ret, Formula = 'Parametric Normal', Confidence_Interval = 0.95 ,
        Period_Interval = None , EWMA_Discount_Factor = 0.94, 
        Series = False ,removeNa = True)
